    ...
    <div className="w-full py-4 bg-white border-b border-gray-100 select-none">
      <div className="flex gap-4 overflow-x-auto px-4 scrollbar-hide scroll-smooth">
        {categories.map((cat, idx) => {
          const thumbSrc = cat.image && cat.image.trim() !== ''
            ? ensureAbsoluteUrl(cat.image)
            : `https://ui-avatars.com/api/?name=${encodeURIComponent(cat.name)}&background=f1f5f9&color=64748b&bold=true&size=128`;
          ...
          return (
            ...
            >
              {/* 1:1 Square Thumbnail Container */}
              <div className="w-full aspect-square rounded-lg overflow-hidden border border-slate-200 flex items-center justify-center bg-slate-50 transition-all duration-300 group-hover:border-black group-hover:shadow-xs group-hover:-translate-y-0.5 relative">
                <img
                  src={thumbSrc}
                  alt={cat.name}
                  loading="eager"
